...
bool Validaciones::validarIsbn(const string &isbn)
{
    ...
    if (all_of(isbnSinGuiones.begin(), isbnSinGuiones.end(), [isbnSinGuiones](char c)
               { return c == isbnSinGuiones[0]; }))
    {
...
bool Validaciones::validarIsni(const string &isni)
{
    ...
    // Verificar que todos los caracteres no sean iguales
    if (all_of(isniSinEspacios.begin(), isniSinEspacios.end(), [isniSinEspacios](char c)
               { return c == isniSinEspacios[0]; }))
    {
